...
st.markdown("""
    <style>
        ...
        .main-header { 
            background-color: #1E3A8A; padding: 1.2rem; border-radius: 0 0 20px 20px; 
            text-align: center; margin-bottom: 2rem;
        }
        .main-header h1 { color: white !important; font-size: 1.8rem; }
        .notice-box { 
            background-color: #DBEAFE; border-left: 5px solid #1E3A8A; 
...
if st.session_state.page == "main":
    ...
    st.write("**✒️ 최종 확인 서명**")
    st_canvas(stroke_width=3, stroke_color="#000", background_color="#f8f9fa", height=130, key="canvas_tbm")

    if st.button("✅ 점검 완료 및 저장"):
        # 에러 방지를 위해 .all() 대신 Python 내장 함수 사용
        is_checked = all(df_common["확인"]) 
        if not final_name or not selected_job or not is_checked:
            st.warning("⚠️ 모든 체크항목을 확인하고 성함을 입력해주세요.")
        elif sheet is None:
            st.error("⚠️ 구글 시트 연결에 실패했습니다. Secrets 설정을 확인하세요.")
        else:
            try:
                kst = timezone(timedelta(hours=9))
                now = datetime.datetime.now(kst)
                # 시트 열 개수에 맞춰 데이터 리스트 생성
