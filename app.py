...
# [4. 스타일 디자인 - 수직 일렬 정중앙 정렬]
st.markdown("""
    <style>
        .stApp { background-color: #F0F8FF; }
        header { visibility: hidden !important; }
        .main-header { background-color: #1E3A8A; padding: 1.5rem 0; border-radius: 0 0 20px 20px; margin-bottom: 2rem; }
        ...
        /* [핵심] 모든 요소를 수직으로 세우고 중앙으로 정렬하는 설정 */
        .stButton, .stMarkdown {
            display: flex;
        ...
        /* 버튼 개별 스타일: 공지 박스와 너비를 맞춤 */
        .stButton > button { 
            width: 100% !important; 
            max-width: 380px; /* 공지 박스 너비와 일치 */
            border-radius: 12px; 
            height: 4.8rem; 
...
if st.session_state.page == "main":
    ...
    # 2. 버튼들 (공지사항 바로 밑으로 주르륵 나열)
    # st.columns를 쓰지 않고 그냥 나열하면 CSS가 중앙으로 정렬합니다.
