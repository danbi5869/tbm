...
# [5. 🌃 Modern Dark Mode 핵심 디자인 CSS]
st.markdown("""
    <style>
        /* 기본 배경: 짙은 다크 네이비 */
        .stApp { background-color: #0c1a32; color: #ffffff; }
        header { visibility: hidden !important; }
        
        /* 메인 헤더: 그라데이션 바 */
        .main-header { 
            background: linear-gradient(135deg, #1e3a8a 0%, #172554 100%);
            padding: 2.5rem 0; border-radius: 0 0 30px 30px; 
            margin-bottom: 2rem; box-shadow: 0 10px 20px rgba(0,0,0,0.3); 
        }
        .main-header h1 { color: #38bdf8 !important; text-align: center; font-size: 2.2rem; margin: 0; font-weight: 800; }
        ...
        /* 메인 버튼 스타일: 네온 블루 테두리와 입체감 */
        .stButton>button { 
            width: 100%; border-radius: 15px; height: 5rem; 
        ...
        div.stButton > button:has(div:contains("메인으로")) { 
            ...
            height: 3rem; border: none !important; 
        ...
        div.stButton > button:has(div:contains("저장하기")) { 
            ...
            height: 4rem; border: none !important; 
        }
        
        /* 공지사항 박스: 정보성이 강한 블루 스타일 */
        .notice-box { 
            background-color: #1e3a8a; 
...
if st.session_state.page == "main":
    st.markdown('<div class="main-header"><h1>⛑️ TBM SAFETY GATE</h1></div>', unsafe_allow_html=True)
    st.write("")
...
# 📝 점검 작성 페이지
    ...
    st.markdown(f'<div class="notice-box"><b><span style="color:#38bdf8; font-size:1.2rem;">📋 금일 안전 지시사항</span></b><br><div style="margin-top:10px;">{display_text}</div></div>', unsafe_allow_html=True)
    
    c1, c2 = st.columns(2)
    with c1: selected_team = st.selectbox("🏗️ 부서 선택", list(team_data.keys()))
    with c2: 
    ...
    selected_job = st.selectbox("🛠️ 금일 작업명", ["", "공통작업", "분해작업", "중량물취급", "전기작업", "세척작업", "조립작업", "시험/가동"])
    ...
    if st.button("🚀 점검 완료 및 저장하기"):
        if not final_name or not selected_job or not df_common["확인"].all():
            st.warning("⚠️ 모든 점검 항목과 성함을 확인해 주세요.")
        else:
            with st.spinner('전송 중...'):
                try:
                    ...
                    st.success("🎉 점검표가 성공적으로 저장되었습니다!"); time.sleep(1.2); st.session_state.page = "main"; st.rerun()
...
# ⚙️ 관리자 페이지
    ...
    if not st.session_state.admin_logged_in:
        ...
        if st.button("업데이트"): st.session_state.safety_notice = new_notice; st.success("공지사항이 변경되었습니다.")
