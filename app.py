...
# 📝 점검 작성 페이지
    ...
    col_config = {
        ...
        "확인": st.column_config.CheckboxColumn("V", width=30)
    }
    common_list = [
        {"작업명": "계획", "점검내용": "작업순서/역할분담 완료", "확인": False},
        {"작업명": "보호구", "점검내용": "안전모/화/장갑 착용", "확인": False},
        {"작업명": "공구", "점검내용": "공구 상태 이상없음", "확인": False},
        {"작업명": "정리", "점검내용": "바닥 장애물 제거", "확인": False},
        {"작업명": "LOTO", "점검내용": "전원차단 확인", "확인": False}
    ]
    
    df_common = st.data_editor(pd.DataFrame(common_list), hide_index=True, use_container_width=True, column_config=col_config, height=210)

    if selected_job and selected_job not in ["", "공통작업"]:
        st.write(f"**⚠️ {selected_job} 추가 점검**")
        st.data_editor(pd.DataFrame(specific_checks[selected_job]), hide_index=True, use_container_width=True, column_config=col_config)

    st.write("**✒️ 최종 확인 서명**")
    st_canvas(stroke_width=2, stroke_color="#000", background_color="#f8f9fa", height=100, width=300, drawing_mode="freedraw", key="canvas_tbm")
...
# ⚙️ 관리자 페이지
    ...
    if not st.session_state.admin_logged_in:
        ...
        new_notice = st.text_area("공지 수정", st.session_state.safety_notice, height=120)
